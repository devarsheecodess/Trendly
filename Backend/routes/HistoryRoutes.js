...
router.get("/scripts", async (req, res) => {
	...
	try {
		if (!id) return res.send({ success: true, scripts: [] });
...
router.get("/seos", async (req, res) => {
	const id = await resolveUserId(req);
	try {
		if (!id) return res.send({ success: true, seos: [] });
...
router.get("/thumbnails", async (req, res) => {
	const id = await resolveUserId(req);
	try {
		if (!id) return res.send({ success: true, thumbnails: [] });
...
router.get("/voiceovers", async (req, res) => {
	const id = await resolveUserId(req);
	try {
		if (!id) return res.send({ success: true, voiceovers: [] });
...
router.get("/youtube", async (req, res) => {
	const id = await resolveUserId(req);
	try {
		if (!id) return res.json({ success: true, videos: [] });
