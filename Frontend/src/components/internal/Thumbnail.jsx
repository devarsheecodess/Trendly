...
const Thumbnail = () => {
    ...
    const handleGenerate = () => {
        ...
        setTimeout(() => {
            setThumbnails([
                { id: 1, url: '/api/placeholder/800/450', alt: 'Thumbnail 1' },
                { id: 2, url: '/api/placeholder/800/450', alt: 'Thumbnail 2' },
                { id: 3, url: '/api/placeholder/800/450', alt: 'Thumbnail 3' }
            ]);
            setIsGenerating(false);
        }, 3000);
    };

    const handleExport = (thumbnailId) => {
        const thumbnail = thumbnails.find(t => t.id === thumbnailId);
        if (!thumbnail) return;

        // In a real app, you would set up a proper download mechanism
    ...
    const handleSave = (thumbnailId) => {
        // Simulate saving to the app
        console.log(`Saving thumbnail ${thumbnailId} to the app`);
        // In a real app, you would make an API call to save the thumbnail
    };
        ...
        <div className="w-full p-3 sm:p-4 md:p-6 bg-stone-50 min-h-screen md:pl-[300px] lg:pl-[300px]">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-stone-200 overflow-hidden">
                <div className="p-3 sm:p-4 md:p-6">
                    <h2 className="text-md sm:text-lg font-medium text-stone-900 mb-3 sm:mb-4">Generated Thumbnails</h2>
                    ...
                    {isGenerating ? (
                        <div className="flex flex-col items-center justify-center text-center py-16 sm:py-24">
                            ...
                            <p className="text-stone-500 text-sm sm:text-base">Generating your thumbnails...</p>
                                    ...
                                    <SwiperSlide key={thumbnail.id}>
                                        <div className="rounded-lg overflow-hidden shadow-md border border-stone-200">
                                            ...
                                            <div className="p-3 sm:p-4 flex flex-col sm:flex-row justify-between">
                                                <span className="text-sm font-medium text-stone-800 mb-2 sm:mb-0">Thumbnail {thumbnail.id}</span>
                                                <div className="flex space-x-2">
                                                    <button
                                                        ...
                                                        onClick={() => handleSave(thumbnail.id)}
                                                    >
                                                    ...
                                                    <button
                                                        className="px-3 py-1.5 text-xs sm:text-sm rounded-md bg-stone-100 text-stone-800 hover:bg-stone-200 transition-colors flex items-center justify-center"
                                                        onClick={() => handleExport(thumbnail.id)}
                                                    >
                                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div className="flex flex-col items-center justify-center text-center py-12 md:py-16">
                            ...
                            <p className="text-stone-500">Enter a topic and click "Generate Thumbnails" to create eye-catching thumbnails</p>
                        </div>
                    )}
                </div>
            </div>
